...
function createHubSpotService({ token, pipelineId, stageNewId, stageCompletedId } = {}) {
  if (!token) throw new Error('createHubSpotService: token is required');
  ...
  return { createTicket, findTicketBySlackTs, getTicket, markListoSent };
}
